...
<div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6 mb-6 border border-gray-200 dark:border-gray-600">
    <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed text-center">
        Enviamos um link de verificação para seu e-mail cadastrado no <strong>{{ config('app.name') }}</strong>. 
...
<div class="space-y-4">
    ...
    <form method="POST" action="{{ route('verification.send') }}">
        @csrf
        <x-primary-button class="w-full justify-center py-3 text-base font-semibold bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500 transform hover:scale-[1.02] transition-all duration-200 shadow-lg hover:shadow-xl">
