    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="text-right">
                {{-- Se quiser mostrar o total de candidatos, precisará passar a variável $totalCandidatos do controller --}}
                {{-- <span class="text-3xl font-bold text-blue-600">{{ $totalCandidatos ?? 0 }}</span>
                <p class="text-xs text-gray-500 uppercase">Total de Candidatos</p> --}}
            </div>
        </div>
    </x-slot>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const cursoOptions = @json($cursos ?? []);
    const instituicaoOptions = @json($instituicoes ?? []);
    ...
    exportPdfBtn.addEventListener('click', function() {
        ...
        form.method = 'POST';
        form.action = '{{ route("admin.candidatos.relatorios.exportar-pdf") }}';
        form.target = '_blank';
