...
class DocumentoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($previousStatus === 'Homologado') {
            ...
            $candidato->revert_reason = array_slice($revertHistory, -5); // Limita o histórico aos últimos 5

            $candidato->save();
            Log::info("Candidato ID {$candidato->id} (Homologado) alterou documento '{$tipoDocumento}' e voltou para 'Em Análise'. Motivo: " . json_encode($candidato->revert_reason));
    ...
    public function destroy(Documento $documento)
    {
        ...
        try {
            ...
            if ($previousStatus === 'Homologado' && !$todosObrigatoriosAindaPresentes) {
                ...
                $revertHistory[] = [
                    'timestamp' => Carbon::now()->toDateTimeString(),
                    'reason' => "Documento obrigatório '{$documento->tipo_documento}' removido pelo candidato.", 
                    'action' => 'document_delete',
                    'document_type' => $documento->tipo_documento,
                    'previous_status' => $previousStatus,
                ];
                $candidato->revert_reason = array_slice($revertHistory, -5); // Limita o histórico
                $candidato->save();
                Log::info("Candidato ID {$candidato->id} (Homologado) removeu documento obrigatório '{$documento->tipo_documento}' e voltou para 'Em Análise'. Motivo: " . json_encode($candidato->revert_reason));
            ...
            elseif (($previousStatus === 'Em Análise' || $previousStatus === 'Aprovado') && !$todosObrigatoriosAindaPresentes) {
                ...
                $candidato->revert_reason = array_slice($revertHistory, -5); // Limita o histórico
                $candidato->save();
                Log::info("Candidato ID {$candidato->id} (Status: {$previousStatus}) removeu documento obrigatório '{$documento->tipo_documento}' e voltou para 'Inscrição Incompleta'. Motivo: " . json_encode($candidato->revert_reason));
