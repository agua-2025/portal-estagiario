...
class ProfileController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            // ✅ CORREÇÃO DEFINITIVA: Aceita o NOME da cidade e verifica se ele existe no estado selecionado.
            'cidade' => [
                'required',
                'string', // Aceita o nome da cidade como texto
                Rule::exists('cidades', 'nome')->where('estado_id', $request->input('estado'))
            ],
        ...
        try {
            ...
            // Mapeia o ID do estado para a coluna de ID correta
            $dataToSave['estado_id'] = $validatedData['estado'];
            
            // Busca o objeto do estado para obter o nome
            $estadoObj = Estado::find($validatedData['estado']);
            if ($estadoObj) {
                // Salva o nome do estado no campo de texto 'estado'
                $dataToSave['estado'] = $estadoObj->nome;
            }

            // Busca a cidade pelo NOME para obter o ID
            $cidadeObj = Cidade::where('nome', $validatedData['cidade'])
                               ->where('estado_id', $dataToSave['estado_id'])
                               ->first();
            if ($cidadeObj) {
                // Mapeia o ID da cidade para a coluna correta
                $dataToSave['cidade_id'] = $cidadeObj->id;
                // O nome da cidade já está correto em $validatedData['cidade']
            }
