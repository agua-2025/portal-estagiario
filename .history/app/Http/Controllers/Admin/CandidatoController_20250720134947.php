    ...
    public function updateDocumentStatus(Request $request, Documento $documento)
    {
        ...
        try {
            ...
            if ($documento->status === 'rejeitado') {
                ...
                if ($candidato) {
                    $candidato->status = 'Inscrição Incompleta';
                    // ✅ AJUSTE CIRÚRGICO: A mensagem geral agora é genérica para evitar inconsistências.
                    $candidato->admin_observacao = "Um ou mais documentos obrigatórios foram rejeitados. Por favor, verifique os detalhes na seção 'Meus Documentos' e faça as correções necessárias.";
                    $candidato->save();
                }
