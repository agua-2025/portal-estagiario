...
<head>
    ...
    <style>
        * {
        ...
        /* Forçar containers a não ultrapassar a tela */
        * {
            max-width: 100%;
...
<body class="antialiased bg-gray-50 overflow-x-hidden">
    ...
    <div class="relative min-h-screen z-10">
        <header class="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-50 header-container" x-data="{ mobileMenuOpen: false }">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
                <div class="flex items-center justify-between py-3 sm:py-4 gap-2 sm:gap-4">
                    <div class="flex items-center min-w-0" data-aos="fade-right">
                        <a href="{{ route('welcome') }}" class="flex items-center space-x-2 sm:space-x-3">
                            <div class="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-gradient-to-br from-blue-600 to-purple-600 rounded-xl flex items-center justify-center flex-shrink-0">
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 md:w-7 md:h-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
                                </svg>
                            </div>
                            <span class="logo-text text-sm sm:text-base md:text-lg lg:text-xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                    ...
                    <!-- Menu Desktop -->
                    <nav class="hidden lg:flex space-x-4 xl:space-x-6" data-aos="fade-down">
                        <a href="{{ route('welcome') }}#cursos" class="text-sm lg:text-base font-medium text-gray-700 hover:text-blue-600 transition-colors duration-200">Cursos</a>
                        <a href="{{ route('welcome') }}#documentos" class="text-sm lg:text-base font-medium text-gray-700 hover:text-blue-600 transition-colors duration-200">Documentos</a>
                        <a href="{{ route('classificacao.index') }}" class="text-sm lg:text-base font-medium text-blue-600 hover:text-blue-800 transition-colors duration-200">Classificação</a>
                        <a href="{{ route('welcome') }}#sobre" class="text-sm lg:text-base font-medium text-gray-700 hover:text-blue-600 transition-colors duration-200">Sobre</a>
                    </nav>

                    <!-- Botões de Auth Desktop -->
                    <div class="hidden sm:flex items-center space-x-1 sm:space-x-2 flex-shrink-0" data-aos="fade-left">
                        @if (Route::has('login'))
                            @auth
                                <a href="{{ url('/dashboard') }}" class="px-3 lg:px-4 py-2 text-xs sm:text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-purple-600 rounded-full hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-lg hover:shadow-xl">Painel</a>
                            @else
                                <a href="{{ route('login') }}" class="text-xs sm:text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors duration-200">Entrar</a>

                                @if (Route::has('register'))
                                    <a href="{{ route('register') }}" class="px-2 sm:px-3 lg:px-4 py-2 text-xs sm:text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-purple-600 rounded-full hover:from-blue-700 hover:to-purple-700 transition-all duration-300 shadow-lg hover:shadow-xl">
                                        Inscreva-se
                                    </a>
                                @endif
                            @endauth
                    ...
                    <button @click="mobileMenuOpen = !mobileMenuOpen" class="sm:hidden p-2 rounded-md text-gray-700 hover:text-blue-600 hover:bg-gray-100 transition-colors duration-200 flex-shrink-0">
