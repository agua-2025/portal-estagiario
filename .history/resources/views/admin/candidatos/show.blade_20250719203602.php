        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8 text-gray-900">
                    ...
                    {{-- ✅ INÍCIO DA ALTERAÇÃO: Bloco de alerta de reanálise inserido aqui --}}
                    @if($candidato->revert_reason && is_array($candidato->revert_reason) && !empty($candidato->revert_reason))
                        @php
                            // Lógica para encontrar o "lote" de alterações que precisam ser revisadas
                            ...
                            foreach ($reversed_history as $event) {
                                ...
                                // O "gatilho" é a primeira alteração que mudou o status de um estado final para análise.
                                // Paramos de coletar quando encontramos esse gatilho.
                                if (in_array($event['previous_status'], ['Homologado', 'Aprovado'])) {
                                    break;
                                }
                            }
                            $changes_to_review = array_reverse($changes_to_review); // Coloca em ordem cronológica novamente

                            // Agora, processamos o lote de alterações para criar um resumo
                            ...
                            foreach ($changes_to_review as $change) {
                                ...
                                if (in_array($change['action'], ['document_update', 'document_delete'])) {
                                    // Adiciona o tipo de documento à lista, sem duplicatas
                                    if (!in_array($change['document_type'], $documents_changed)) {
                                        $documents_changed[] = $change['document_type'];
                                    }
                                }
                            }
                            $first_change = $changes_to_review[0] ?? null;
                        @endphp

                        @if($first_change)
                            <div class="my-4 p-4 border-l-4 border-yellow-500 bg-yellow-100 text-yellow-800 rounded-lg" role="alert">
                                <h3 class="font-bold text-lg mb-2">Atenção: Inscrição Reanalisada</h3>
                                <p>
                                    Esta inscrição retornou para o status <strong>"Em Análise"</strong> em <strong>{{ \Carbon\Carbon::parse($first_change['timestamp'])->format('d/m/Y \à\s H:i') }}</strong> (status anterior: <strong>{{ $first_change['previous_status'] }}</strong>) devido às seguintes alterações feitas pelo candidato:
                                </p>
                                
                                <ul class="list-disc pl-5 mt-3 font-semibold">
                                    @if($profile_updated)
                                        <li>O Perfil do Candidato foi modificado.</li>
                                    @endif
                                    @if(!empty($documents_changed))
                                        <li>
                                            Os seguintes documentos foram alterados/enviados:
                                            <p class="font-normal bg-white p-2 mt-1 rounded border border-yellow-200">{{ implode(', ', $documents_changed) }}</p>
                                        </li>
                                    @endif
                                </ul>

                                <details class="mt-4 text-sm">
                                    <summary class="cursor-pointer text-blue-600 hover:underline">Ver detalhes e histórico completo</summary>
                                    <ul class="list-disc pl-5 mt-2">
                                        @foreach (array_reverse($candidato->revert_reason) as $event)
                                            <li>
                                                <strong>{{ \Carbon\Carbon::parse($event['timestamp'])->format('d/m/Y H:i') }}:</strong> 
                                                {{ $event['reason'] }} (Status anterior: {{ $event['previous_status'] }})
                                            </li>
                                        @endforeach
                                    </ul>
                                </details> 
                        ...
                        <div class="mt-8 pt-6 border-t p-4 bg-gray-100 rounded-lg">
                           <h3 class="text-lg font-semibold text-gray-800 mb-2">Painel de Ações do Administrador</h3>
                            ...
                            @if ($candidato->status === 'Em Análise')
                                {{-- O alerta de reversão agora está no topo da página, então aqui mostramos uma mensagem padrão --}}
