            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 md:p-8 text-gray-800">
                    ...
                    <div x-show="tab === 'perfil'" x-transition.opacity>
                        ...
                        <div class="mt-8 pt-6 border-t border-gray-200 p-4 border border-gray-100 rounded-lg bg-gray-50">
                        ...
                        <div class="space-y-5">
                            ...
                            @foreach($documentosNecessarios as $tipoDocumento => $nomeDocumento)
                                ...
                                <div class="p-5 border border-gray-200 rounded-lg flex flex-col sm:flex-row justify-between items-start gap-4 text-sm bg-white shadow-sm">
                                    ...
                                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 flex-shrink-0"> {{-- Ajuste para stack em mobile e row em desktop --}}
                                        @if($documentoEnviado)
                                            <a href="{{ route('candidato.documentos.show', $documentoEnviado) }}" target="_blank" class="w-full sm:w-auto px-4 py-2 bg-gray-800 text-white rounded-md text-sm hover:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-700 focus:ring-offset-2 transition-colors duration-200 text-center">Visualizar</a> {{-- Cor mais escura, largura total em mobile --}}
                                            
                                            @if($documentoEnviado->status !== 'aprovado')
                                                <form action="{{ route('admin.documentos.updateStatus', $documentoEnviado) }}" method="POST" onsubmit="return confirm('Aprovar este documento?');" class="w-full sm:w-auto"> {{-- Adicionado w-full em mobile --}}
                                                    @csrf
                                                    @method('PUT')
                                                    <input type="hidden" name="status" value="aprovado">
                                                    <button type="submit" class="w-full px-4 py-2 bg-green-700 text-white rounded-md text-sm hover:bg-green-800 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-offset-2 transition-colors duration-200">Aprovar</button> {{-- Cor mais escura --}}
                                                </form>
                                            @endif

                                            @if($documentoEnviado->status !== 'rejeitado')
                                                <button @click="showDocRejectionModal = true; docRejectionAction = '{{ route('admin.documentos.updateStatus', $documentoEnviado) }}'" type="button" class="w-full sm:w-auto px-4 py-2 bg-red-700 text-white rounded-md text-sm hover:bg-red-800 focus:outline-none focus:ring-2 focus:ring-red-600 focus:ring-offset-2 transition-colors duration-200">Rejeitar</button> {{-- Cor mais escura --}}
                                            @endif
                                        @else
                                            <span class="text-xs text-gray-500 italic w-full sm:w-auto text-center sm:text-left">Aguardando envio do candidato.</span> {{-- Centralizado em mobile --}}
                                        @endif
                                    </div>
                                </div>
                            @endforeach
                            ...
                            @foreach($candidato->user->candidatoAtividades as $atividade)
                                <div class="p-5 border border-gray-200 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center text-sm bg-white shadow-sm">
                                    ...
                                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 flex-shrink-0"> {{-- Ajuste para stack em mobile e row em desktop --}}
                                        <a href="{{ route('candidato.atividades.show', $atividade) }}" target="_blank" class="w-full sm:w-auto px-4 py-2 bg-gray-800 text-white rounded-md text-sm hover:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-700 focus:ring-offset-2 transition-colors duration-200 text-center">Visualizar</a>
                                        
                                        @if ($atividade->status !== 'Aprovada')
                                            <form action="{{ route('admin.atividades.aprovar', $atividade->id) }}" method="POST" onsubmit="return confirm('Aprovar este item?');" class="w-full sm:w-auto">
                                                @csrf
                                                <button type="submit" class="w-full px-4 py-2 bg-green-700 text-white rounded-md text-sm hover:bg-green-800 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-offset-2 transition-colors duration-200">Aprovar Item</button>
                                            </form>
                                        @endif

                                        @if ($atividade->status !== 'Rejeitada')
                                            <button @click="showRejectionModal = true; rejectionAction = '{{ route('admin.atividades.rejeitar', $atividade->id) }}'" type="button" class="w-full sm:w-auto px-4 py-2 bg-red-700 text-white rounded-md text-sm hover:bg-red-800 focus:outline-none focus:ring-2 focus:ring-red-600 focus:ring-offset-2 transition-colors duration-200">Rejeitar Item</button>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div x-show="tab === 'acoes'" x-transition.opacity x-cloak>
                        <div class="mt-8 pt-6 border-t border-gray-200 p-6 bg-gray-50 rounded-lg shadow-sm">
                                    ...
                                    <form action="{{ route('admin.candidatos.update', $candidato->id) }}" method="POST">
                                        ...
                                        <div class="space-y-6">
                                            ...
                                            <div class="flex flex-wrap items-center gap-4">
                                                <button type="submit" name="status" value="Aprovado" class="px-6 py-2 bg-green-700 text-white rounded-lg text-base font-semibold hover:bg-green-800 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-offset-2 transition-colors duration-200">Aprovar Inscrição</button> {{-- Cor mais escura --}}
                                                <button @click="showProfileRejectionModal = true" type="button" class="px-6 py-2 bg-red-700 text-white rounded-lg text-base font-semibold hover:bg-red-800 focus:outline-none focus:ring-2 focus:ring-red-600 focus:ring-offset-2 transition-colors duration-200">Rejeitar Inscrição</button> {{-- Cor mais escura --}}
                                            </div>
                                        </div>
                                    </form>
                                @elseif ($candidato->status === 'Aprovado')
            ...
            <div @click.away="showRejectionModal = false" class="bg-white rounded-xl shadow-2xl p-7 w-full max-w-lg mx-auto">
                ...
                <form :action="rejectionAction" method="POST">
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        ...
                        <button type="submit" class="px-5 py-2.5 bg-red-700 text-white rounded-lg hover:bg-red-800 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-red-600 focus:ring-offset-2 transition-colors duration-200">Confirmar Rejeição</button> {{-- Cor mais escura --}}
                    </div>
                </form>
            </div>
        </div>
            ...
            <div @click.away="showProfileRejectionModal = false" class="bg-white rounded-xl shadow-2xl p-7 w-full max-w-lg mx-auto">
                ...
                <form action="{{ route('admin.candidatos.update', $candidato->id) }}" method="POST">
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        ...
                        <button type="submit" class="px-5 py-2.5 bg-red-700 text-white rounded-lg hover:bg-red-800 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-red-600 focus:ring-offset-2 transition-colors duration-200">Confirmar Rejeição da Inscrição</button> {{-- Cor mais escura --}}
                    </div>
                </form>
            </div>
        </div>
