<x-app-layout>
    ...
    <div class="py-12 bg-gray-50"> {{-- Fundo da seção principal mais suave --}}
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 md:p-8 text-gray-800">
                        ...
                        <div class="space-y-5">
                            ...
                            @foreach($documentosNecessarios as $tipoDocumento => $nomeDocumento)
                                ...
                                <div class="p-5 border border-gray-200 rounded-lg flex flex-col sm:flex-row justify-between items-start gap-4 text-sm bg-white shadow-sm">
                                    ...
                                    <div class="flex-grow">
                                        {{-- ✅ AJUSTE: Mover status para o topo, antes do título --}}
                                        @if($documentoEnviado)
                                    ...
                                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 flex-shrink-0 min-w-[100px] sm:min-w-[unset]"> {{-- Adicionado min-width para consistência em mobile --}}
                                        @if($documentoEnviado)
                                            <a href="{{ route('candidato.documentos.show', $documentoEnviado) }}" target="_blank" class="w-full sm:w-28 px-4 py-2 bg-gray-600 text-white rounded-md text-sm hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors duration-200 text-center">Visualizar</a> {{-- Botão com largura fixa em desktop, nova cor --}}
                                            
                                            @if($documentoEnviado->status !== 'aprovado')
                                                <form action="{{ route('admin.documentos.updateStatus', $documentoEnviado) }}" method="POST" onsubmit="return confirm('Aprovar este documento?');" class="w-full sm:w-28"> {{-- Largura fixa --}}
                                                    @csrf
                                                    @method('PUT')
                                                    <input type="hidden" name="status" value="aprovado">
                                                    <button type="submit" class="w-full px-4 py-2 bg-green-600 text-white rounded-md text-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors duration-200">Aprovar</button> {{-- Nova cor --}}
                                                </form>
                                            @endif

                                            @if($documentoEnviado->status !== 'rejeitado')
                                                <button @click="showDocRejectionModal = true; docRejectionAction = '{{ route('admin.documentos.updateStatus', $documentoEnviado) }}'" type="button" class="w-full sm:w-28 px-4 py-2 bg-red-600 text-white rounded-md text-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors duration-200">Rejeitar</button> {{-- Nova cor --}}
                                            @endif
                                        @else
                            ...
                            @foreach($candidato->user->candidatoAtividades as $atividade)
                                <div class="p-5 border border-gray-200 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center text-sm bg-white shadow-sm">
                                    ...
                                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 flex-shrink-0 min-w-[100px] sm:min-w-[unset]"> {{-- Largura fixa em desktop --}}
                                        <a href="{{ route('candidato.atividades.show', $atividade) }}" target="_blank" class="w-full sm:w-28 px-4 py-2 bg-gray-600 text-white rounded-md text-sm hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors duration-200 text-center">Visualizar</a>
                                        ...
                                        @if ($atividade->status !== 'Aprovada')
                                            <form action="{{ route('admin.atividades.aprovar', $atividade->id) }}" method="POST" onsubmit="return confirm('Aprovar este item?');" class="w-full sm:w-28">
                                                @csrf
                                                <button type="submit" class="w-full px-4 py-2 bg-green-600 text-white rounded-md text-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors duration-200">Aprovar Item</button>
                                            </form>
                                        @endif

                                        @if ($atividade->status !== 'Rejeitada')
                                            <button @click="showRejectionModal = true; rejectionAction = '{{ route('admin.atividades.rejeitar', $atividade->id) }}'" type="button" class="w-full sm:w-28 px-4 py-2 bg-red-600 text-white rounded-md text-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors duration-200">Rejeitar Item</button>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div x-show="tab === 'acoes'" x-transition.opacity x-cloak>
                        <div class="mt-8 pt-6 border-t border-gray-200 p-6 bg-white rounded-lg shadow-sm"> {{-- Fundo branco para a área de ações --}}
