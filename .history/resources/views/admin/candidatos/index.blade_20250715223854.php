            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($candidatos as $candidato)
                        <tr>
                            <td class="px-6 py-4 text-sm font-medium text-gray-900"> {{-- ✅ REMOVIDO: whitespace-nowrap --}}
                                {{ $candidato->nome_completo }}
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-500"> {{-- ✅ REMOVIDO: whitespace-nowrap --}}
                                {{ $candidato->curso->nome ?? 'N/A' }} {{-- Nome do curso genérico --}}
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-500"> {{-- ✅ REMOVIDO: whitespace-nowrap --}}
                                {{ $candidato->instituicao->nome ?? 'N/A' }} {{-- Nome da instituição diretamente do candidato --}}
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-500"> {{-- ✅ REMOVIDO: whitespace-nowrap --}}
                                {{ $candidato->status }}
                            </td>
                            <td class="px-6 py-4 text-right text-sm font-medium"> {{-- ✅ REMOVIDO: whitespace-nowrap --}}
