<x-app-layout>
    ...
    <div class="py-12" x-data="{ tab: 'analise', showRejectionModal: false, rejectionAction: '', showScoreDetails: false, showProfileRejectionModal: false, showDocRejectionModal: false, docRejectionAction: '' }">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg"> {{-- Sombra mais pronunciada e bordas levemente arredondadas --}}
                <div class="p-6 md:p-8 text-gray-800"> {{-- Texto mais escuro para melhor legibilidade --}}

                    {{-- CABEÇALHO COM STATUS --}}
                    <div class="flex flex-col sm:flex-row justify-between items-start mb-6 border-b border-gray-200 pb-4"> {{-- Borda mais suave --}}
                        <div>
                            <h2 class="text-3xl font-bold text-gray-900 leading-tight">{{ $candidato->nome_completo ?? $candidato->user->name }}</h2> {{-- Título maior e mais impactante --}}
                        ...
                        <div class="mt-4 sm:mt-0 flex flex-col sm:flex-row items-center gap-4">
                            ...
                            @endphp
                            <span class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full {{ $statusClass }}"> {{-- Adicionado ring para um visual mais premium --}}
                                {{ $statusText }}
                            </span>
                            <a href="{{ route('admin.candidatos.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2 transition ease-in-out duration-150"> {{-- Botão de voltar mais sutil --}}
                                Voltar
                            </a>
                        </div>
                    </div>
                    ...
                    @if($profile_was_updated)
                        <div class="mb-6 p-4 border-l-4 border-yellow-400 bg-yellow-50 text-yellow-800 rounded-lg text-sm" role="alert"> {{-- Cor de alerta mais suave --}}
                    ...
                    {{-- PLACAR DE PONTUAÇÃO --}}
                    <div class="mb-6 p-5 bg-blue-50 border border-blue-200 rounded-lg shadow-sm"> {{-- Padding maior e sombra sutil --}}
                        <div class="flex justify-between items-center">
                            <div>
                                <h3 class="text-sm font-medium text-blue-700">Pontuação Total (Itens Aprovados)</h3> {{-- Tom de azul mais suave --}}
                                <p class="mt-1 text-4xl font-extrabold text-blue-900">{{ number_format($pontuacaoTotal, 2, ',', '.') }} pontos</p> {{-- Fonte mais ousada --}}
                            </div>
                            <button @click="showScoreDetails = !showScoreDetails" class="text-sm text-blue-600 hover:text-blue-800 hover:underline transition-colors duration-200"> {{-- Transição suave para o hover --}}
                        ...
                        <div x-show="showScoreDetails" x-transition.opacity class="mt-4 pt-4 border-t border-blue-200">
                            <h4 class="text-xs font-semibold text-gray-600 uppercase mb-3">Extrato de Pontos</h4> {{-- Margem e cor ajustadas --}}
                            <table class="min-w-full text-sm">
                                <tbody class="divide-y divide-blue-100"> {{-- Linhas divisórias suaves --}}
                                    @forelse($detalhesPontuacao as $detalhe)
                                    <tr class="hover:bg-blue-50 transition-colors duration-150"> {{-- Efeito hover na linha --}}
                                        <td class="py-2 pr-4 text-gray-700">{{ $detalhe['nome'] }}</td>
                                        <td class="py-2 pl-4 text-right font-semibold text-gray-900">{{ number_format($detalhe['pontos'], 2, ',', '.') }}</td> {{-- Fonte semibold --}}
                                    </tr>
                                    @empty
                                    <tr>
                                        <td class="py-2 text-gray-500 italic">Nenhuma atividade foi aprovada ainda.</td> {{-- Texto em itálico para "nenhuma atividade" --}}
                                    </tr>
                                    @endforelse
                                </tbody>
                            </table>
                    ...
                    <div class="border-b border-gray-200 mb-6">
                        <nav class="-mb-px flex space-x-6" aria-label="Tabs">
                            <button @click="tab = 'perfil'" :class="{ 'border-blue-600 text-blue-700 font-semibold': tab === 'perfil', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': tab !== 'perfil' }" class="whitespace-nowrap py-3 px-1 border-b-2 text-sm focus:outline-none transition-colors duration-200"> {{-- Cores e transições refinadas --}}
                                Perfil do Candidato
                            </button>
                    ...
                    <div x-show="tab === 'perfil'" x-transition.opacity> {{-- Transição suave ao trocar de aba --}}
                        @php
                        function renderDetail($label, $value) {
                            ...
                            echo '<div class="mb-4">
                                  ...
                                  </div>'; // Estilo mais profissional para os detalhes
                        }
                        @endphp
                        <div class="mt-6 p-4 border border-gray-100 rounded-lg bg-gray-50"> {{-- Card para dados pessoais --}}
                            <h3 class="text-xl font-semibold text-gray-800 mb-5">Dados Pessoais</h3>
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6"> {{-- Mais colunas em telas maiores, maior espaçamento --}}
                        ...
                        <div class="mt-8 pt-6 border-t border-gray-200 p-4 border border-gray-100 rounded-lg bg-gray-50"> {{-- Card para dados acadêmicos --}}
                    ...
                    <div x-show="tab === 'analise'" x-transition.opacity style="display: none;">
                        <div class="space-y-5"> {{-- Espaçamento maior entre os itens --}}
                            ...
                            @foreach($documentosNecessarios as $tipoDocumento => $nomeDocumento)
                                ...
                                @endphp
                                <div class="p-5 border border-gray-200 rounded-lg flex flex-col sm:flex-row justify-between items-start gap-4 text-sm bg-white shadow-sm"> {{-- Mais padding, borda e sombra mais elegantes --}}
                                    {{-- Informações do Documento --}}
                                    <div class="flex-grow">
                                        <div class="flex items-center mb-1">
                                            <p class="font-bold text-gray-800 text-base">{{ $nomeDocumento }}</p> {{-- Fonte mais ousada --}}
                                            {{-- ✅ AJUSTE: O 'ALTERADO' só aparece se o documento ainda não foi aprovado pelo admin --}}
                                            @if(in_array($tipoDocumento, $changed_document_types) && $documentoEnviado && $documentoEnviado->status !== 'aprovado')
                                                <span class="ml-3 px-2.5 py-0.5 bg-yellow-100 text-yellow-700 rounded-full text-xs font-semibold ring-1 ring-yellow-200">ALTERADO</span> {{-- Estilo mais profissional para tag --}}
                                            @endif
                                        </div>
                                        @if($documentoEnviado)
                                            ...
                                            @if($documentoEnviado->status == 'rejeitado' && $documentoEnviado->motivo_rejeicao)
                                                <p class="text-xs text-red-600 mt-2">Motivo: {{ $documentoEnviado->motivo_rejeicao }}</p> {{-- Cor mais suave para o motivo da rejeição --}}
                                            @endif
                                        @else
                                            <span class="text-xs font-medium capitalize px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 ring-1 ring-gray-200">Status: Pendente</span>
                                        @endif
                                    </div>
                                    
                                    {{-- Ações do Admin para o Documento --}}
                                    <div class="flex items-center space-x-2 flex-shrink-0 mt-3 sm:mt-0">
                                        @if($documentoEnviado)
                                            <a href="{{ route('candidato.documentos.show', $documentoEnviado) }}" target="_blank" class="px-4 py-2 bg-gray-700 text-white rounded-md text-sm hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors duration-200">Visualizar</a> {{-- Botões com transição e focus ring --}}
                                            ...
                                            @if($documentoEnviado->status !== 'aprovado')
                                                <form action="{{ route('admin.documentos.updateStatus', $documentoEnviado) }}" method="POST" onsubmit="return confirm('Aprovar este documento?');">
                                                    ...
                                                    <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded-md text-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors duration-200">Aprovar</button>
                                                </form>
                                            @endif

                                            @if($documentoEnviado->status !== 'rejeitado')
                                                <button @click="showDocRejectionModal = true; docRejectionAction = '{{ route('admin.documentos.updateStatus', $documentoEnviado) }}'" type="button" class="px-4 py-2 bg-red-600 text-white rounded-md text-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors duration-200">Rejeitar</button>
                                            @endif
                                        @else
                                            <span class="text-xs text-gray-500 italic">Aguardando envio do candidato.</span>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
                                ...
                                <div class="p-5 border border-gray-200 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center text-sm bg-white shadow-sm">
                                    <div class="flex-grow mb-3 sm:mb-0">
                                        <div class="flex items-center mb-1">
                                            ...
                                            @endphp
                                            <span class="font-medium capitalize px-2.5 py-1 rounded-full text-xs mr-3 {{ $statusClassAtividade }}">{{ $atividade->status }}</span>
                                        ...
                                        @if($atividade->status === 'Rejeitada' && $atividade->motivo_rejeicao)
                                            <div class="text-xs text-red-600 mt-2 p-3 bg-red-50 rounded-md border border-red-100"> {{-- Card mais destacado para motivo de rejeição --}}
                                    ...
                                    <div class="flex items-center space-x-2 flex-shrink-0 mt-3 sm:mt-0">
                                        <a href="{{ route('candidato.atividades.show', $atividade) }}" target="_blank" class="px-4 py-2 bg-gray-700 text-white rounded-md text-sm hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors duration-200">Visualizar</a>
                                        ...
                                        @if ($atividade->status !== 'Aprovada')
                                            <form action="{{ route('admin.atividades.aprovar', $atividade->id) }}" method="POST" onsubmit="return confirm('Aprovar este item?');">
                                                @csrf
                                                <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded-md text-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors duration-200">Aprovar Item</button>
                                            </form>
                                        @endif

                                        @if ($atividade->status !== 'Rejeitada')
                                            <button @click="showRejectionModal = true; rejectionAction = '{{ route('admin.atividades.rejeitar', $atividade->id) }}'" type="button" class="px-4 py-2 bg-red-600 text-white rounded-md text-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors duration-200">Rejeitar Item</button>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div x-show="tab === 'acoes'" x-transition.opacity x-cloak>
                        <div class="mt-8 pt-6 border-t border-gray-200 p-6 bg-gray-50 rounded-lg shadow-sm"> {{-- Padding, fundo e sombra --}}
                            ...
                            @else
                                @if ($candidato->status === 'Em Análise')
                                    ...
                                    <form action="{{ route('admin.candidatos.update', $candidato->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="space-y-6"> {{-- Espaçamento maior --}}
                                            <div>
                                                <label for="admin_observacao" class="block text-sm font-medium text-gray-700 mb-1">Justificativa / Observação</label>
                                                <textarea name="admin_observacao" id="admin_observacao" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 transition duration-150 ease-in-out">{{ $candidato->admin_observacao }}</textarea> {{-- Estilo de input melhorado --}}
                                            </div>
                                            <div class="flex flex-wrap items-center gap-4"> {{-- Flex-wrap para melhor responsividade --}}
                                                <button type="submit" name="status" value="Aprovado" class="px-6 py-2 bg-green-600 text-white rounded-lg text-base font-semibold hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors duration-200">Aprovar Inscrição</button>
                                                <button @click="showProfileRejectionModal = true" type="button" class="px-6 py-2 bg-red-600 text-white rounded-lg text-base font-semibold hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors duration-200">Rejeitar Inscrição</button>
                                            </div>
                                        </div>
                                    </form>
                                @elseif ($candidato->status === 'Aprovado')
                                    <form action="{{ route('admin.candidatos.homologar', $candidato->id) }}" method="POST" class="w-full mt-4">
                                        @csrf
                                        <div class="bg-blue-50 p-5 rounded-lg mt-4 mb-4 border border-blue-200 shadow-sm"> {{-- Card mais destacado e profissional para homologação --}}
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <textarea name="homologacao_observacoes" id="homologacao_observacoes" rows="4" 
                                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 transition duration-150 ease-in-out">{{ old('homologacao_observacoes', $candidato->homologacao_observacoes) }}</textarea>
                                ...
                                @else
                                    <div class="bg-gray-100 p-5 rounded-lg mt-4 mb-4 border border-gray-200 shadow-sm"> {{-- Fundo mais claro para status neutro --}}
        ...
        <div x-show="showRejectionModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="fixed inset-0 bg-gray-900 bg-opacity-60 flex items-center justify-center z-50 p-4" style="display: none;"> {{-- Efeitos de transição melhorados e padding no modal container --}}
            <div @click.away="showRejectionModal = false" class="bg-white rounded-xl shadow-2xl p-7 w-full max-w-lg mx-auto"> {{-- Sombra mais profunda e bordas mais arredondadas --}}
                ...
                <form :action="rejectionAction" method="POST">
                    ...
                    <div>
                        ...
                        <textarea name="motivo_rejeicao" id="motivo_rejeicao" rows="5" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 transition duration-150 ease-in-out" required minlength="10" placeholder="Ex: Documento ilegível, dados inconsistentes, etc."></textarea> {{-- Placeholder adicionado --}}
                    </div>
                    <div class="mt-6 flex justify-end space-x-3">
                        ...
                        <button type="submit" class="px-5 py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors duration-200">Confirmar Rejeição</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div @click.away="showProfileRejectionModal = false" class="bg-white rounded-xl shadow-2xl p-7 w-full max-w-lg mx-auto">
                ...
                <form action="{{ route('admin.candidatos.update', $candidato->id) }}" method="POST">
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        ...
                        <button type="submit" class="px-5 py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors duration-200">Confirmar Rejeição da Inscrição</button>
                    </div>
                </form>
            </div>
        </div>
