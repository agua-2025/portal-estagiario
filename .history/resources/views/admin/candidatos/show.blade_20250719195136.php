        ...
        <div class="flex flex-col sm:flex-row justify-between items-start mb-6 border-b pb-4">
            ...
            <div class="mt-4 sm:mt-0 flex flex-col sm:flex-row items-center gap-4">
                ...
                <a href="{{ route('admin.candidatos.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300">
                    ...
                    {{-- Aba 3: Ações Finais (onde a lógica de homologação e ações entrará) --}}
                    <div x-show="tab === 'acoes'" x-transition x-cloak>
                        ...
                        <div class="mt-8 pt-6 border-t p-4 bg-gray-100 rounded-lg">
                           <h3 class="text-lg font-semibold text-gray-800 mb-2">Painel de Ações do Administrador</h3>
                            ...
                            {{-- Mensagens de status e formulários de ação --}}
                            @if ($candidato->status === 'Em Análise')
                                {{-- ✅ NOVO: Exibe o histórico de motivos da reversão se o status voltou para 'Em Análise' --}}
                                @php
                                    // Garante que $revertReasons seja sempre um array, mesmo se revert_reason for null ou string antiga
                                    $revertReasons = is_array($candidato->revert_reason) ? $candidato->revert_reason : [];
                                @endphp
                                @if (!empty($revertReasons)) {{-- Verifica se o array não está vazio --}}
                                    <div class="bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4 mb-4" role="alert">
                                        <p class="font-bold">Status Revertido para "Em Análise"!</p>
                                        <p class="text-sm mt-1 mb-2">O candidato realizou as seguintes alterações que exigem nova análise:</p>
                                        <ul class="list-disc list-inside text-sm">
                                            @foreach ($revertReasons as $revert)
                                                <li>
                                                    <span class="font-medium">{{ \Carbon\Carbon::parse($revert['timestamp'])->format('d/m/Y H:i') }}</span>: {{ $revert['reason'] }}
                                                    @if (isset($revert['action'])) (Ação: {{ $revert['action'] }}) @endif
                                                </li>
                                            @endforeach
                                        </ul>
                                        <p class="text-sm mt-2">Por favor, revise as alterações e decida sobre a aprovação.</p>
                                    </div>
                                @else
                                    <p class="text-sm text-gray-700 mb-4">O candidato está aguardando sua análise. Por favor, revise o perfil e os documentos/atividades.</p>
                                @endif
                            ...
                            @elseif ($candidato->status === 'Homologado')
                                <div class="bg-blue-50 p-4 rounded-lg mt-4 mb-4">
                                    ...
                                    @endif
                                    {{-- ✅ NOVO: Exibe o histórico de motivos da reversão --}}
                                    @php
                                        $revertReasons = is_array($candidato->revert_reason) ? $candidato->revert_reason : [];
                                    @endphp
                                    @if (!empty($revertReasons))
                                        <div class="mt-4 pt-3 border-t border-blue-200 text-sm text-blue-700">
                                            <p class="font-bold">Histórico de Reversões de Status:</p>
                            ...
                            @else {{-- Status como 'Rejeitado', 'Inscrição Incompleta' --}}
                                <div class="bg-gray-50 p-4 rounded-lg mt-4 mb-4">
                                    ...
                                    {{-- ✅ NOVO: Exibe o motivo da reversão se o status voltou para Inscrição Incompleta --}}
                                    @php
                                        $revertReasons = is_array($candidato->revert_reason) ? $candidato->revert_reason : [];
                                    @endphp
                                    @if (!empty($revertReasons))
                                        <p class="text-sm text-red-600 mt-2">Motivo da Reversão:</p>
