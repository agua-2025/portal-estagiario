            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        function profileFormComponent(initialFields, estados, todasCidades, instituicoes, todosCursos, totalProfileFields, completableFields, candidatoStatus) {
                            return {
                                ...
                                init() {
                                    ...
                                    // ✅ CORREÇÃO: Ajusta o watch para naturalidade_estado para limpar naturalidade_cidade se o ID não for encontrado
                                    this.$watch('fields.naturalidade_estado', () => { 
                                        if (this.fields.naturalidade_estado) {
                                            ...
                                            // Verifica se o ID da cidade atual existe nas cidades filtradas
                                            const cidadeAtualExiste = cidadesDoEstado.find(c => c.id == this.fields.naturalidade_cidade);
                                            if (!cidadeAtualExiste) {
                                                this.fields.naturalidade_cidade = ''; // Limpa o campo se o ID não for válido para o novo estado
                                            }
                                        } else {
                                            this.fields.naturalidade_cidade = ''; // Limpa se o estado for removido
                                        }
                                        this.updateAllFilteredLists(); 
                                    });

                                    // ✅ CORREÇÃO: Ajusta o watch para estado para limpar cidade se o ID não for encontrado
                                    this.$watch('fields.estado', () => { 
                                        if (this.fields.estado) {
                                            ...
                                            // Verifica se o ID da cidade atual existe nas cidades filtradas
                                            const cidadeAtualExiste = cidadesDoEstado.find(c => c.id == this.fields.cidade);
                                            if (!cidadeAtualExiste) {
                                                this.fields.cidade = ''; // Limpa o campo se o ID não for válido para o novo estado
                                            }
                                        } else {
                                            this.fields.cidade = ''; // Limpa se o estado for removido
                                        }
                                        this.updateAllFilteredLists(); 
                                    });
                                ...
                                isStep1Valid() { 
                                    return !!this.fields.nome_completo && 
                                            ...
                                            // ✅ CORREÇÃO: Valida se naturalidade_cidade é um número e não está vazio
                                            !!this.fields.naturalidade_cidade && !isNaN(parseInt(this.fields.naturalidade_cidade)) &&
                                ...
                                isStep2Valid() { 
                                    return !!this.fields.telefone && 
                                            ...
                                            !!this.fields.bairro && 
                                            !!this.fields.estado && 
                                            // ✅ CORREÇÃO: Valida se cidade é um número e não está vazio
                                            !!this.fields.cidade && !isNaN(parseInt(this.fields.cidade)); 
                                },
                                ...
                                isInvalid(field, step) {
                                    ...
                                    // ✅ CORREÇÃO: Para campos de ID (cidade_id, estado_id, etc.), verifica se é um número válido.
                                    if (['cidade', 'naturalidade_cidade', 'estado', 'naturalidade_estado', 'instituicao_id', 'curso_id'].includes(field)) {
                                ...
                                calculatePercentage() {
                                    ...
                                    completableFields.forEach(field => {
                                        ...
                                        // ✅ CORREÇÃO: Para campos de ID, verifica se é um número válido antes de considerar preenchido.
                                        else if (['cidade_id', 'estado_id', 'instituicao_id', 'curso_id'].includes(field)) {
                    ...
                    @php
                        ...
                        // ✅ CORREÇÃO: Garante que os IDs sejam strings para o Alpine.js e que os campos de cidade/estado usem os IDs
                        $initialData['naturalidade_estado'] = old('naturalidade_estado', (string) $candidato->naturalidade_estado_id); // Usar naturalidade_estado_id
                        $initialData['estado'] = old('estado', (string) $candidato->estado_id); // Usar estado_id
                        ...
                        // ✅ CORREÇÃO: Garante que as cidades sejam IDs (string) para o Alpine.js
                        $initialData['naturalidade_cidade'] = old('naturalidade_cidade', $candidato->naturalidade_cidade_id ? (string) $candidato->naturalidade_cidade_id : ''); // Usar naturalidade_cidade_id
                        $initialData['cidade'] = old('cidade', $candidato->cidade_id ? (string) $candidato->cidade_id : ''); // Usar cidade_id
                    @endphp
                                    ...
                                    <div class="col-span-12 sm:col-span-8">
                                        ...
                                        <select :class="{ 'border-red-500': isInvalid('naturalidade_cidade', 1) }" x-model="fields.naturalidade_cidade" name="naturalidade_cidade" id="naturalidade_cidade" class="mt-1 block w-full rounded-md shadow-sm border-gray-300" required :disabled="isAguardandoHomologacao"> {{-- ✅ DESABILITADO --}}
                                            ...
                                            <template x-for="cidade_item in cidadesNaturalidadeFiltradas" :key="cidade_item.id">
                                                {{-- ✅ CORREÇÃO AQUI: Envia o ID da cidade --}}
                                                <option :value="cidade_item.id" x-text="cidade_item.nome" :selected="cidade_item.id == fields.naturalidade_cidade"></option>
                                            </template>
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-span-12 sm:col-span-8">
                                        ...
                                        <select :class="{ 'border-red-500': isInvalid('cidade', 2) }" x-model="fields.cidade" name="cidade" id="cidade" class="mt-1 block w-full rounded-md shadow-sm border-gray-300" required :disabled="isAguardandoHomologacao"> {{-- ✅ DESABILITADO --}}
                                            ...
                                            <template x-for="cidade_item in cidadesEnderecoFiltradas" :key="cidade_item.id">
                                                {{-- ✅ CORREÇÃO AQUI: Envia o ID da cidade --}}
                                                <option :value="cidade_item.id" x-text="cidade_item.nome" :selected="cidade_item.id == fields.cidade"></option>
                                            </template>
                                        </select>
                                    </div>
                                </div>
