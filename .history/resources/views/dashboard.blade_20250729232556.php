...
{{-- ✅ ADICIONAR O PAINEL DE ALERTAS GLOBAIS AQUI --}}
@php
    ...
    if ($candidato) {
        ...
        $perfilIncompleto = !$candidato->isProfileComplete();
        $inscricaoRejeitada = $candidato->status === 'Rejeitado';
        $correcaoSolicitada = $candidato->status === 'Inscrição Incompleta' && !empty($candidato->admin_observacao);
        
        $temPendencias = $documentosRejeitados->count() > 0 || 
                         $atividadesRejeitadas->count() > 0 || 
                         $perfilIncompleto || 
                         $inscricaoRejeitada || 
                         $correcaoSolicitada;
    } else {
...
@if($candidato && $temPendencias)
    <div class="mb-8 p-5 border-l-4 border-yellow-500 bg-gradient-to-r from-yellow-50 to-yellow-100 rounded-r-lg shadow-md">
        <div class="flex items-start">
            <div class="flex-shrink-0">
                {{-- Ícone de alerta (triângulo) --}}
                <svg class="h-7 w-7 text-yellow-600" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
                </svg>
            </div>
            <div class="ml-4 flex-1">
                <h3 class="text-lg font-bold text-yellow-800 mb-2">
                    ⚠️ Atenção! Você possui pendências que precisam ser resolvidas
                </h3>
                <div class="text-sm text-yellow-700 space-y-2">
                    ...
                    @if($perfilIncompleto)
                        <div class="flex items-center space-x-2">
                            ...
                            <span><strong>Perfil Incompleto:</strong> Complete seus dados cadastrais.</span>
                            <a href="{{ route('candidato.profile.edit') }}" class="inline-flex items-center text-yellow-800 hover:text-yellow-900 font-semibold underline ml-2">
                                Completar agora
                    ...
                    @if($correcaoSolicitada)
                        <div class="flex items-center space-x-2">
                            <span class="w-2 h-2 bg-red-500 rounded-full"></span>
                            <span><strong>Correção Solicitada:</strong> A comissão solicitou ajustes no seu cadastro.</span>
                        </div>
                    @endif

                    @if($inscricaoRejeitada)
                        <div class="flex items-center space-x-2">
                            <span class="w-2 h-2 bg-red-500 rounded-full"></span>
                            <span><strong>Inscrição Rejeitada:</strong> Sua inscrição foi rejeitada. Verifique os motivos.</span>
                        </div>
                    @endif
                    ...
                    @if($documentosRejeitados->count() > 0)
                        <div class="flex items-center space-x-2">
                            <span class="w-2 h-2 bg-red-500 rounded-full"></span>
                            <span><strong>{{ $documentosRejeitados->count() }} Documento(s) Rejeitado(s):</strong> Corrija os documentos.</span>
                            <a href="{{ route('candidato.documentos.index') }}" class="inline-flex items-center text-yellow-800 hover:text-yellow-900 font-semibold underline ml-2">
                                Ver documentos
                                <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"></path>
                                </svg>
                    ...
                    @if($atividadesRejeitadas->count() > 0)
                        <div class="flex items-center space-x-2">
                            <span class="w-2 h-2 bg-red-500 rounded-full"></span>
                            <span><strong>{{ $atividadesRejeitadas->count() }} Atividade(s) Rejeitada(s):</strong> Verifique as atividades.</span>
                            <a href="{{ route('candidato.atividades.index') }}" class="inline-flex items-center text-yellow-800 hover:text-yellow-900 font-semibold underline ml-2">
                                Ver atividades
                                <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"></path>
                                </svg>
                ...
                <div class="mt-4">
                    @php
                        $totalItens = 4; // perfil + documentos + atividades + inscrição
                        $itensOk = 0;
                        if(!$perfilIncompleto) $itensOk++;
                        if($documentosRejeitados->count() == 0) $itensOk++;
                        if($atividadesRejeitadas->count() == 0) $itensOk++;
                        if(!$inscricaoRejeitada && !$correcaoSolicitada) $itensOk++;
                        $progresso = ($itensOk / $totalItens) * 100;
                    @endphp
                    <div class="flex items-center">
                        <span class="text-xs text-yellow-700 mr-2 font-medium">Resolução das pendências:</span>
                        <div class="flex-1 bg-yellow-200 rounded-full h-3 overflow-hidden">
                            <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 h-3 rounded-full transition-all duration-500 ease-out" style="width: {{ $progresso }}%"></div>
                        </div>
                        <span class="text-xs text-yellow-700 ml-2 font-bold">{{ round($progresso) }}%</span>
                    </div>
                </div>
            </div>
        </div>
...
@endif
{{-- ✅ FIM DO PAINEL DE ALERTAS GLOBAIS --}}
